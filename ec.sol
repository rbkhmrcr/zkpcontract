contract ECLib {
    ...
    function ecDouble(uint256 x1, uint256 y1, uint256 z1) constant
        returns(uint256 x3, uint256 y3, uint256 z3) {
    ...
    function ecMul(uint256 d, uint256 x1,uint256 y1,uint256 z1) constant
        returns(uint256 x3, uint256 y3, uint256 z3) {
        uint256 remaining = d;
        uint256 px = x1;
        uint256 py = y1;
    ...
    function publicKey(uint256 privKey) constant
        ...
        // Multiplies private key by group generator to give public key
        uint256 x;
        uint256 y;
        uint256 z;
        (x,y,z) = ecMul(privKey, gx, gy, 1);
        z = _inverse(z);
        qx = mulmod(x , z ,n);
        qy = mulmod(y , z ,n);
    }
    ...
    function deriveKey(uint256 privKey, uint256 pubX, uint256 pubY) constant
        returns(uint256 qx, uint256 qy) {
        // This could be used to compute shared secrets, Diffie-Hellman style? :)
        uint256 x;
        uint256 y;
        uint256 z;
        (x, y, z) = ecMul(privKey, pubX, pubY, 1);
        z = _inverse(z);
        qx = mulmod(x, z, n);
        qy = mulmod(y, z, n);
    }
